...
export default function Tracey({
    ...
    const scheduleNext = () => {
      ...
      window.setTimeout(() => {
        ...
        if (Math.random() < 0.65) {
          ...
          setTilt(Math.random() < 0.5 ? -4 : 4);
          window.setTimeout(() => !cancelled && setTilt(0), 400);
        }
        scheduleNext();
      }, delay);
    };
  ...
  return (
    <motion.svg
      ...
      style={{ transformOrigin: "60px 130px", overflow: "visible" }}
    >
      <defs>
        {/* 3D body gradient — top-left light source */}
        ...
        {/* glossy highlight on upper body */}
      ...
      <motion.ellipse
        ...
        style={{ transformOrigin: "18px 90px" }}
      ...
      <motion.ellipse
        ...
        style={{ transformOrigin: "102px 90px" }}
        transition={{ duration: 0.3 }}
      />

      {/* body — 3D tapered bell with radial gradient */}
      <path
      ...
      <path
        ...
        fill="url(#t3dGloss)"
      />
      {/* specular highlight — bright spot top-left of body */}
      <ellipse cx="46" cy="50" rx="12" ry="8" fill="#ffffff" opacity="0.18" />
      {/* rim light — right edge */}
      <path
        d="M90 50 C96 64 98 86 92 108"
        stroke="#ffffff"
        strokeWidth="2"
        fill="none"
        opacity="0.08"
      ...
      {/* eyes — 3D with gradient whites and glossy pupils */}
      {expression !== "cool" && (
        <>
          {eyesClosed ? (
            <>
      ...
      {/* detecting: 3D magnifying glass + radar */}
