//! trace-daemon: the local API server and dashboard host.
//!
//! Exposed as a library so the single `trace` binary can host the daemon in a
//! child process — release artifacts ship only `trace`. A thin `main.rs` is kept
//! for `cargo run -p trace-daemon` during development.

pub mod api;
pub mod assets;
pub mod cloud_sync;
pub mod intel_routes;
pub mod server;
pub mod state;
