...
//! 3. `~/.traceguard/github.json` → `{ "token": "..." }`.
//!
//! This is read-only. TraceGuard never pushes, and a token is only ever sent to
//! `api.github.com` over HTTPS — never to TraceGuard's own surfaces.
...
mod tests {
    ...
    fn parses_https_remote() {
        let r = parse_remote("https://github.com/TaxCollector23/TraceGuard.git").unwrap();
        assert_eq!(r.owner, "TaxCollector23");
        assert_eq!(r.repo, "TraceGuard");
    }
