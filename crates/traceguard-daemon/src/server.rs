...
pub async fn serve(preferred_port: u16) -> Result<()> {
    ...
    let started_at = traceguard_core::time::now_rfc3339();
    ...
    tracing::info!("TraceGuard daemon listening on http://127.0.0.1:{port}");
    println!("TraceGuard daemon listening on http://127.0.0.1:{port}");
