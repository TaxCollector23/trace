...
fn detach(cmd: &mut Command) {
    ...
    // DETACHED_PROCESS: the child gets no console at all, so it can't be
    // killed by the parent's console closing. CREATE_NEW_PROCESS_GROUP:
    // Ctrl+C/Ctrl+Break sent to the parent's console does not propagate to
    // the child (Windows delivers those to the whole console process group
    // by default). Together these are the Windows equivalent of the
    // setsid() call above — a background daemon that outlives the CLI.
