...
pub fn run(opts: RunOptions) -> Result<()> {
    ...
    let (exit_code, output) = exec_tee(&command, &root, &log_dir, &run_id, port)?;
...
fn exec_tee(
    ...
    let mut child = shell_command(command)
        .current_dir(cwd)
        // So the agent's own hooks (e.g. Claude Code's PostToolUse hook,
        // see integrations/claude/trace-hook.sh) can reach the daemon and
        // tag events against this specific run without any extra config.
        .env("TRACE_RUN_ID", run_id)
        .env("TRACE_DAEMON_PORT", daemon_port.to_string())
        .stdin(Stdio::inherit())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
