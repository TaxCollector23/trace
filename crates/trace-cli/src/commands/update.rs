...
pub fn run() -> Result<()> {
    ...
    // Windows refuses to overwrite (or rename over) the exe file this very
    // process is running from — the mapping holds an exclusive lock on
    // in-place writes. Renaming it *aside* first is allowed (that doesn't
    // need exclusive access), then the new binary can take its name. Unix
    // has no such restriction: renaming straight over a running binary
    // works because the running process holds the old inode, not the path.
    #[cfg(windows)]
    {
        let old = dir.join(".trace-update.old");
        let _ = std::fs::remove_file(&old); // best-effort cleanup from a prior update
