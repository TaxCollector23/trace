...
use trace_core::{git, paths, time::now_rfc3339, ProjectConfig};
...
pub fn run() -> Result<()> {
    ...
    if config_path.exists() {
        println!("Trace is already initialized in this project.");
    ...
    println!("Initialized Trace project \"{project_name}\".");
    println!("  config:   {}", config_path.display());
    ...
    if git::is_git_repo(&cwd) {
        ...
        println!("Checkpoints and rollback require Git. Trace will NOT");
        println!("initialize Git for you — run `git init` yourself if you want");
        println!("checkpoint/rollback support.");
    }

    println!("\nNext: run an agent through Trace, e.g. `trace run \"claude\"`.");
    Ok(())
}
